...
#define EXO_PARAM_WRITABLE (G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB)
...
GType exo_tree_view_get_type(void) {
    ...
    if (G_UNLIKELY(type == G_TYPE_INVALID)) {
        type = _exo_g_type_register_simple(GTK_TYPE_TREE_VIEW, "ExoTreeView", sizeof(ExoTreeViewClass),
                                           exo_tree_view_class_init, sizeof(ExoTreeView), exo_tree_view_init);
    }
...
static void exo_tree_view_class_init(ExoTreeViewClass* klass) {
    ...
    g_object_class_install_property(
        gobject_class, PROP_SINGLE_CLICK,
        g_param_spec_boolean("single-click", _("Single Click"),
                             _("Whether the items in the view can be activated with single clicks"), FALSE,
                             EXO_PARAM_READWRITE));
    ...
    g_object_class_install_property(gobject_class, PROP_SINGLE_CLICK_TIMEOUT,
                                    g_param_spec_uint("single-click-timeout", _("Single Click Timeout"),
                                                      _("The amount of time after which the item under the mouse "
                                                        "cursor will be selected automatically in single click mode"),
                                                      0, G_MAXUINT, 0, EXO_PARAM_READWRITE));
     ...
     **/
    g_object_class_install_property(
        gobject_class, PROP_MIDDLE_CLICK,
        g_param_spec_boolean("middle-click", _("Middle Click"),
                             _("Whether the items in the view can be activated with middle clicks"), FALSE,
                             EXO_PARAM_READWRITE));
}
...
static gboolean exo_tree_view_button_release_event(GtkWidget* widget, GdkEventButton* event) {
    ...
    if (G_LIKELY(event->window == gtk_tree_view_get_bin_window(GTK_TREE_VIEW(tree_view)))) {
        ...
        if (G_UNLIKELY(tree_view->priv->single_click && tree_view->priv->button_release_activates)) {
            ...
            if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(tree_view), event->x, event->y, &path, &column, NULL,
                                              NULL)) {
...
#endif
        {
            ...
            if ((path == NULL && tree_view->priv->hover_path != NULL) ||
                ...
                if (G_LIKELY(tree_view->priv->single_click_timeout > 0 && tree_view->priv->hover_path != NULL)) {
                    ...
                    tree_view->priv->single_click_timeout_id = gdk_threads_add_timeout_full(
                        G_PRIORITY_LOW, tree_view->priv->single_click_timeout, exo_tree_view_single_click_timeout,
                        tree_view, exo_tree_view_single_click_timeout_destroy);
                }
            }
            else {
    ...
    if (!g_source_is_destroyed(g_main_current_source()))
        ...
        if (gtk_widget_has_focus(GTK_WIDGET(tree_view)) && tree_view->priv->single_click &&
            ...
            if (model != NULL && gtk_tree_model_get_iter(model, &iter, tree_view->priv->hover_path)) {
                ...
                gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(tree_view), tree_view->priv->hover_path, cursor_column,
                                             FALSE, 0.0f, 0.0f);
                ...
                if (gtk_tree_selection_get_mode(selection) == GTK_SELECTION_NONE) {
                    ...
                    gtk_tree_view_set_cursor(GTK_TREE_VIEW(tree_view), tree_view->priv->hover_path, cursor_column,
                                             FALSE);
                }
                    ...
                    if (!gtk_tree_selection_path_is_selected(selection, tree_view->priv->hover_path)) {
                        ...
                        if (G_UNLIKELY(cursor_path == NULL)) {
                            ...
                            gtk_tree_view_set_cursor(GTK_TREE_VIEW(tree_view), tree_view->priv->hover_path,
                                                     cursor_column, FALSE);
                        }
                        else {
