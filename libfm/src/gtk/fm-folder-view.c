...
static GtkToggleActionEntry folder_toggle_actions[] = {
    ...
    /* Note to translators: "Mingle..." means "Do not put folders before files" but make the translation as short as
       possible, please! */
...
static void fm_folder_view_default_init(FmFolderViewInterface* iface) {
     ...
     * Since: 0.1.0
     */
    signals[CLICKED] = g_signal_new("clicked", FM_TYPE_FOLDER_VIEW, G_SIGNAL_RUN_FIRST,
                                    G_STRUCT_OFFSET(FmFolderViewInterface, clicked), NULL, NULL,
     ...
     * Since: 0.1.0
     */
    signals[SEL_CHANGED] = g_signal_new("sel-changed", FM_TYPE_FOLDER_VIEW, G_SIGNAL_RUN_FIRST,
                                        G_STRUCT_OFFSET(FmFolderViewInterface, sel_changed), NULL, NULL,
     ...
     * Since: 0.1.10
     */
    signals[SORT_CHANGED] = g_signal_new("sort-changed", FM_TYPE_FOLDER_VIEW, G_SIGNAL_RUN_FIRST,
                                         G_STRUCT_OFFSET(FmFolderViewInterface, sort_changed), NULL, NULL,
     ...
     * Since: 1.0.2
     */
    signals[FILTER_CHANGED] = g_signal_new("filter-changed", FM_TYPE_FOLDER_VIEW, G_SIGNAL_RUN_FIRST,
                                           G_STRUCT_OFFSET(FmFolderViewInterface, filter_changed), NULL, NULL,
                                           g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
     ...
     * Since: 1.2.0
     */
    signals[COLUMNS_CHANGED] = g_signal_new("columns-changed", FM_TYPE_FOLDER_VIEW, G_SIGNAL_RUN_FIRST,
                                            G_STRUCT_OFFSET(FmFolderViewInterface, columns_changed), NULL, NULL,
                                            g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
void fm_folder_view_set_model(FmFolderView* fv, FmFolderModel* model) {
    ...
    if (old_model) {
        ...
        /* https://bugs.launchpad.net/ubuntu/+source/oneg4fm/+bug/1071231:
...
static void on_create_new(GtkAction* act, FmFolderView* fv) {
    ...
    if (templ == NULL) /* new folder or empty file */
    {
        ...
        if (!prompt)
            prompt = _prompt = g_strdup_printf(_("Enter a name for the new %s:"), fm_mime_type_get_desc(mime_type));
    ...
    basename = fm_get_user_input_n(GTK_WINDOW(win), header, prompt, name_template, n, run_button);
    if (templ) {
        g_free(_prompt);
        g_free(header);
    }
...
static void on_paste(GtkAction* act, FmFolderView* fv) {
    ...
    if (FOCUS_IS_IN_FOLDER_VIEW(focus, GTK_WIDGET(fv))) {
        ...
        g_debug("paste on %s isn't supported by FmFolderView widget", G_OBJECT_TYPE_NAME(focus));
}
...
static void popup_position_func(GtkMenu* menu, gint* x, gint* y, gboolean* push_in, gpointer user_data) {
    ...
    gdk_window_get_device_position(
        ...
        gdk_device_manager_get_client_pointer(gdk_display_get_device_manager(gdk_screen_get_display(screen))), &x2, &y2,
        NULL);
...
static void on_menu(GtkAction* act, FmFolderView* fv) {
    ...
    gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(act), (mode & FM_SORT_CASE_SENSITIVE) == 0);
    act = gtk_ui_manager_get_action(ui, "/popup/Sort/MingleDirs");
    gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(act), (mode & FM_SORT_NO_FOLDER_FIRST) != 0);
...
static gboolean on_key_press(GtkWidget* widget, GdkEventKey* evt, FmFolderView* fv) {
    ...
    if ((evt->keyval == GDK_KEY_Menu && !modifier) || (evt->keyval == GDK_KEY_F10 && modifier == GDK_SHIFT_MASK)) {
...
static inline GtkMenu* _make_file_menu(FmFolderView* fv,
                                       GtkWindow* win,
    ...
    if (update_popup)
        update_popup(fv, win, fm_file_menu_get_ui(menu), fm_file_menu_get_action_group(menu), files);
    ...
    if (G_LIKELY(scheme)) {
        ...
        for (l = extensions; l; l = l->next) {
            ...
            if (ext->cb.update_file_menu_for_scheme != NULL)
                ext->cb.update_file_menu_for_scheme(win, fm_file_menu_get_ui(menu), str,
                                                    fm_file_menu_get_action_group(menu), menu, files, single_file);
        }
        if (str->len > 0)
            gtk_ui_manager_add_ui_from_string(fm_file_menu_get_ui(menu), str->str, str->len, NULL);
 ...
 * Since: 1.0.1
 */
GtkMenu* fm_folder_view_add_popup(FmFolderView* fv, GtkWindow* parent, FmFolderViewUpdatePopup update_popup) {
    FmFolderViewInterface* iface;
    FmFolderModel* model;
    GtkUIManager* ui;
    GtkActionGroup* act_grp;
    ...
    gtk_action_group_add_actions(act_grp, folder_popup_actions, G_N_ELEMENTS(folder_popup_actions), fv);
    gtk_action_group_add_toggle_actions(act_grp, folder_toggle_actions, G_N_ELEMENTS(folder_toggle_actions), fv);
    gtk_action_group_add_radio_actions(act_grp, folder_sort_type_actions, G_N_ELEMENTS(folder_sort_type_actions), type,
                                       G_CALLBACK(on_change_type), fv);
    gtk_action_group_add_radio_actions(act_grp, folder_sort_by_actions, G_N_ELEMENTS(folder_sort_by_actions), by,
                                       G_CALLBACK(on_change_by), fv);
 ...
 * Since: 1.0.1
 */
void fm_folder_view_item_clicked(FmFolderView* fv, GtkTreePath* path, FmFolderViewClickType type) {
    FmFolderViewInterface* iface;
    GtkTreeModel* model;
    FmFileInfo* fi;
    FmFileInfoList* files;
    ...
    if (open_folders)
        switch (type) {
            case FM_FV_ACTIVATED: /* file(s) activated */
                ...
                fm_launch_files_simple(win, NULL, fm_file_info_list_peek_head_link(files), open_folders, win);
                fm_file_info_list_unref(files);
                break;
