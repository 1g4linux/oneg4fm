...
static gboolean update_scroll(gpointer data) {
    ...
    gtk_adjustment_set_value(gtk_scrolled_window_get_vadjustment(scroll), scroll_pos);
...
static gboolean open_search_func(GAppLaunchContext* ctx, GList* folder_infos, gpointer user_data, GError** err) {
    ...
    FmFileInfo* fi = (FmFileInfo*)l->data;
    GSList* cols = NULL;
    const FmFolderViewColumnInfo col_infos[] = {{FM_FOLDER_MODEL_COL_NAME},
...
static void update_files_popup(FmFolderView* fv,
                               GtkWindow* win,
                               GtkUIManager* ui,
    ...
    g_object_set_qdata_full(G_OBJECT(act_grp), fm_qdata_id, fm_file_info_list_ref(files),
                            (GDestroyNotify)fm_file_info_list_unref);
    gtk_ui_manager_add_ui_from_string(ui, folder_menu_xml, -1, NULL);
}

static void update_sidebar_popup(FmSidePane* sp,
                                 GtkUIManager* ui,
                                 GtkActionGroup* act_grp,
                                 FmFileInfo* file,
                                 gpointer win) {
    ...
    gtk_action_group_add_actions(act_grp, folder_menu_actions, G_N_ELEMENTS(folder_menu_actions), win);
    g_object_set_qdata_full(G_OBJECT(act_grp), fm_qdata_id, files, (GDestroyNotify)fm_file_info_list_unref);
    gtk_ui_manager_add_ui_from_string(ui, folder_menu_xml, -1, NULL);
}
...
static void on_sel_changed(FmFolderView* fv, gint n_files, FmMainWin* win) {
    ...
    if (n_files) {
        ...
        if (n_files == 1) /* only one file is selected */
        {
            ...
            if (size_str) {
                msg = g_strdup_printf("\"%s\" (%s) %s", fm_file_info_get_disp_name(fi), size_str ? size_str : "",
                                      fm_file_info_get_desc(fi));
            }
            else {
                msg = g_strdup_printf("\"%s\" %s", fm_file_info_get_disp_name(fi), fm_file_info_get_desc(fi));
            }
        }
        else
...
static void on_history_item(GtkMenuItem* mi, FmMainWin* win) {
    ...
    int scroll_pos =
        gtk_adjustment_get_value(gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(win->folder_view)));
...
static void fm_main_win_init(FmMainWin* win) {
    ...
    gtk_action_group_add_radio_actions(act_grp, main_win_sort_type_actions, G_N_ELEMENTS(main_win_sort_type_actions),
                                       GTK_SORT_ASCENDING, G_CALLBACK(on_sort_type), win);
    gtk_action_group_add_radio_actions(act_grp, main_win_sort_by_actions, G_N_ELEMENTS(main_win_sort_by_actions), 0,
                                       G_CALLBACK(on_sort_by), win);
    gtk_action_group_add_radio_actions(act_grp, main_win_pathbar_actions, G_N_ELEMENTS(main_win_pathbar_actions), 0,
                                       G_CALLBACK(on_path_mode), win);
    ...
    for (i = 0; i < fm_standard_view_get_n_modes(); i++) {
        if (fm_standard_view_get_mode_label(i)) {
            ...
            mode_action =
                gtk_radio_action_new(str->str, fm_standard_view_get_mode_label(i), fm_standard_view_get_mode_tooltip(i),
                                     fm_standard_view_get_mode_icon(i), i);
            gtk_radio_action_set_group(mode_action, radio_group);
            radio_group = gtk_radio_action_get_group(mode_action);
            gtk_action_group_add_action_with_accel(act_grp, GTK_ACTION(mode_action), accel_str);
...
static void on_folder_fs_info(FmFolder* folder, FmMainWin* win) {
    ...
    if (fm_folder_get_filesystem_info(folder, &total, &free)) {
        ...
        g_snprintf(buf, G_N_ELEMENTS(buf), "Free space: %s (Total: %s)", free_str, total_str);
