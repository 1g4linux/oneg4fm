...
void FileTransferJob::gfileCopyProgressCallback(goffset current_num_bytes,
                                                goffset total_num_bytes,
                                                FileTransferJob* _this) {
...
bool FileTransferJob::copySpecialFile(const FilePath& srcPath, const GFileInfoPtr& srcInfo, FilePath& destPath) {
    ...
    if (!ret) {
        GErrorPtr err;
        g_set_error(&err, G_IO_ERROR, G_IO_ERROR_FAILED, ("Cannot copy file '%s': not supported"),
...
bool FileTransferJob::copyDirContent(const FilePath& srcPath, GFileInfoPtr /*srcInfo*/, FilePath& destPath, bool skip) {
    ...
    GErrorPtr err;
    auto enu =
        GFileEnumeratorPtr{g_file_enumerate_children(srcPath.gfile().get(), defaultGFileInfoQueryAttribs,
    ...
    do {
        ...
        if (!mkdir_done) {
            if (err->domain == G_IO_ERROR &&
                (err->code == G_IO_ERROR_EXISTS || err->code == G_IO_ERROR_INVALID_FILENAME ||
                 err->code == G_IO_ERROR_FILENAME_TOO_LONG)) {
                GFileInfoPtr destInfo =
                    GFileInfoPtr{g_file_query_info(destPath.gfile().get(), defaultGFileInfoQueryAttribs,
                                                   G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable().get(), nullptr),
                                 false};
                if (!destInfo) {
    ...
    if (mkdir_done && !isCancelled()) {
        ...
        if (mode) {
            ...
            do {
                ...
                chmod_done =
                    g_file_set_attribute_uint32(destPath.gfile().get(), G_FILE_ATTRIBUTE_UNIX_MODE, mode,
                                                G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable().get(), &err);
                if (!chmod_done) {
...
bool FileTransferJob::handleError(GErrorPtr& err,
                                  const FilePath& srcPath,
                                  const GFileInfoPtr& srcInfo,
                                  FilePath& destPath,
                                  int& flags) {
    bool retry = false;
    /* handle existing files or file name conflict */
    if (err.domain() == G_IO_ERROR && (err.code() == G_IO_ERROR_EXISTS || err.code() == G_IO_ERROR_INVALID_FILENAME ||
                                       err.code() == G_IO_ERROR_FILENAME_TOO_LONG)) {
...
bool FileTransferJob::processPath(const FilePath& srcPath, const FilePath& destDirPath, const char* destFileName) {
    ...
    GFileInfoPtr srcInfo =
        GFileInfoPtr{g_file_query_info(srcPath.gfile().get(), defaultGFileInfoQueryAttribs,
                                       G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, cancellable().get(), &err),
                     false};
    ...
    switch (mode_) {
        case Mode::MOVE:
            ret =
                moveFile(srcPath, srcInfo, destDirPath, hasDestDirPath_ && destCopyName ? destCopyName : destFileName);
            break;
        case Mode::COPY: {
            bool deleteSrc = false;
            ret = copyFile(srcPath, srcInfo, destDirPath, hasDestDirPath_ && destCopyName ? destCopyName : destFileName,
                           deleteSrc);
            break;
        }
        case Mode::LINK:
...
bool FileTransferJob::moveFile(const FilePath& srcPath,
                               const GFileInfoPtr& srcInfo,
                               const FilePath& destDirPath,
                               const char* destFileName) {
    setCurrentFile(srcPath);

    GErrorPtr err;
    GFileInfoPtr destDirInfo =
        GFileInfoPtr{g_file_query_info(destDirPath.gfile().get(), "id::filesystem", G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
                                       cancellable().get(), &err),
                     false};
...
bool FileTransferJob::copyFile(const FilePath& srcPath,
                               const GFileInfoPtr& srcInfo,
                               const FilePath& destDirPath,
                               const char* destFileName,
                               bool skip) {
        ...
        switch (file_type) {
            case G_FILE_TYPE_DIRECTORY:
                ...
                if (srcPath.isPrefixOf(destPath)) {
                    GErrorPtr err =
                        GErrorPtr{G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, tr("Cannot copy a directory into itself!")};
                    emitError(err, ErrorSeverity::MODERATE);
                }
                else {
...
bool FileTransferJob::linkFile(const FilePath& srcPath,
                               const GFileInfoPtr& srcInfo,
                               const FilePath& destDirPath,
                               const char* destFileName) {
    setCurrentFile(srcPath);
    ...
    if (kf) {
        ...
        if (content) {
            ...
            do {
                ...
                if (!g_file_replace_contents(destPath.gfile().get(), content.get(), contentLen, nullptr, false,
                                             G_FILE_CREATE_NONE, nullptr, cancellable().get(), &err)) {
                    retry = handleError(err, srcPath, srcInfo, destPath, flags);
                    err.reset();
                }
                else {
...
void FileTransferJob::exec() {
    ...
    for (size_t i = 0; i < srcPaths_.size(); ++i) {
        ...
        if (mode_ != Mode::LINK) {
            ...
            runCoreRoutedPath(srcPath, destPath, srcRouting, destRouting);
            continue;
        }
#endif
