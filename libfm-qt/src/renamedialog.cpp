...
RenameDialog::RenameDialog(const FileInfo& src, const FileInfo& dest, QWidget* parent, Qt::WindowFlags f)
    : QDialog(parent, f), action_(ActionIgnore), applyToAll_(false) {
    ...
    if (!disp_size.isEmpty()) {
        infoStr = QString(tr("Type: %1\nSize: %2\nModified: %3")).arg(src.description(), disp_size, srcMtime);
    }
    else {
        infoStr = QString(tr("Type: %1\nModified: %2")).arg(src.description(), srcMtime);
    }
    ...
    if (!disp_size.isEmpty()) {
        infoStr = QString(tr("Type: %1\nSize: %2\nModified: %3")).arg(dest.description(), disp_size, destMtime);
    }
    else {
        infoStr = QString(tr("Type: %1\nModified: %2")).arg(dest.description(), destMtime);
    }
