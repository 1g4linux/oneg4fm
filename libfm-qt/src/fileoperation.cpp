...
FileOperation::FileOperation(Type type, Fm::FilePathList srcPaths, QObject* parent)
    ...
    if (job_) {
        ...
        connect(job_, &Fm::FileOperationJob::fileExists, this, &Fm::FileOperation::onJobFileExists,
                Qt::BlockingQueuedConnection);

        // we block the job deliberately until we prepare to start (initiailize the timer) so we can calculate elapsed
        // time correctly.
        connect(job_, &Fm::FileOperationJob::preparedToRun, this, &Fm::FileOperation::onJobPrepared,
                Qt::BlockingQueuedConnection);
    }
}
...
void FileOperation::onJobFileExists(const FileInfo& src,
                                    const FileInfo& dest,
                                    Fm::FileOperationJob::FileExistsAction& response,
                                    FilePath& newDest) {
    pauseElapsedTimer();
    showDialog();
...
void FileOperation::onJobFinish() {
    ...
    if (type_ == Trash && !job_->isCancelled()) {
        ...
        if (!unsupportedFiles.empty()) { /* delete them instead */
            ...
            if (QMessageBox::question(pWidget ? pWidget->window() : nullptr, tr("Error"),
...
FileOperation* FileOperation::deleteFiles(Fm::FilePathList srcFiles, bool prompt, QWidget* parent) {
    if (prompt && !srcFiles.empty()) {
        int result = QMessageBox::warning(parent ? parent->window() : nullptr, tr("Confirm"),
...
FileOperation* FileOperation::trashFiles(Fm::FilePathList srcFiles, bool prompt, QWidget* parent) {
    if (prompt && !srcFiles.empty()) {
        int result =
            QMessageBox::warning(parent ? parent->window() : nullptr, tr("Confirm"),
